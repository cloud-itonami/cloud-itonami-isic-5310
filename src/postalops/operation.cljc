(ns postalops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (PostalOpsAdvisor)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the PostalOpsGovernor (:govern) and the rollout phase gate
  ...
  human postal operations coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [postalops.advisor :as advisor]
            [postalops.governor :as governor]
...
(defn build
  ...
  `postalops.store/Store`).
  opts:
    :advisor      -- a `postalops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PostalOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; PostalOpsGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
